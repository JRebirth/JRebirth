...
public final class DiagramModel extends DefaultModel<DiagramModel, DiagramView> implements ModuleModel {

    /** The class logger. */
    private static final Logger LOGGER = LoggerFactory.getLogger(DiagramModel.class);
     ...
     */
    public void addShapeModel(final Model createdModel) {
        view().getEditor().getChildren().add(createdModel.node());
    }
     ...
     * @param createdNode the created node
     */
    public void removeShapeModel(final Model createdModel) {
        view().getEditor().getChildren().remove(createdModel.node());
    }
