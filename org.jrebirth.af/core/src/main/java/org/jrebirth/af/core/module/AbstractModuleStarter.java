...
public abstract class AbstractModuleStarter implements ModuleStarter {
    ...
    protected void register(final Class<? extends Component<?>> interfaceClass, final Class<? extends Component<?>> implClass, final PriorityLevel priority, final int weigh) {

        preloadClass(interfaceClass);

        getFacade().componentFactory().register(RegistrationItem.create().interfaceClass(interfaceClass).implClass(implClass));
    }
